...
sigma = covariance;
RPB = chol(sigma);
zPBi = repmat(m1,500,1) + randn(500,2)*RPB;
m2 = mB;
zPBii = repmat(m2,500,1) + randn(500,2)*RPB;
...
plot(xorio,yorio);
hold on
scatter(zPBi(:,1),zPBi(:,2));   
hold on
scatter(zPBii(:,1),zPBii(:,2));
title('Fig3: Contour plots for classes A & B pdfs with the desicion boundary and 500 test samples from each class')
legend({'Class A','Class B','Decision Boundary','Class A Samples','Class B Samples'},'Location','northeast','Orientation','horizontal','NumColumns',1)
hold off
